...
public class ZbyteSupportProgress extends JApplet implements PageFinishedListener 
{
	...
	public void init() {
		...
		try {
			...
			deliverable_select.setVisibleColumns(new String[] {"Select", "id", "Barcode", "Image File", "Copy Date", "Control Num:", "Line #", "Support Type:", "Media:", "Job:", "Operator:"
					, "Comment"});
...
//			c.add(tabbedPane);
	...
	//	frame.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
		...
		ZbyteSupportProgress gj = new ZbyteSupportProgress();
	...
	public void pageFinished(WizardPage currentPage) {
		SupportTransferItemFactory supportTransferItemFactory = new SupportTransferItemFactory();
		if (currentPage == deliverable_select) {
			deliverable_select.setFinishDate(getTodaysDate());
			ArrayList<TableData> selectedItems = deliverable_select.loadSelectedItems(supportTransferItemFactory);
		...
		if (currentPage == deliverable_summary) {
			if (deliverable_select.getChanged()) {
				...
				try {
					...
					AbstractSpreadsheetModel m = deliverable_summary.buildCreatedDeliverableModel(supportTransferItemFactory, model);
