...
public class SeismicTrace implements Comparable<SeismicTrace>
{
    ...
    //this version of set headers takes a long string as input and a separator string.
    ...
    public String toString()
    {
        ...
        for (i = 0; i < headerList.length - 1; i++)
        {
            string += String.format(FORMAT,
                i, headerList[i].trim(), headers[i], i, data[i]);
        }
        string += String.format(FORMAT,
            i, headerList[i].trim(), headers[i], data.length - 1, data[data.length - 1]);
        return string;
    }
